...
//РАБОТА С ФАЙЛОМ
// Вспомогательная функция для добавления одного города (возвращает true, если добавлен)
bool TForm1::AddCityFromCoords(float x, float y)
{
    // Проверка на дублирование координат
    for (int i = 0; i < (int)cities.size(); i++) {
        if (cities[i].x == x && cities[i].y == y) {
...
{
 if (!OpenDialog1->Execute()) return; // пользователь отменил
    ...
    if (!file.is_open()) {
        StatusBar1->SimpleText = "Не удалось открыть файл!";
        return;
    }
    ...
    while (std::getline(file, line)) {
        ...
        float x, y;
        if (iss >> x >> y) {
            // Попытка добавить город
			if (AddCityFromCoords(x, y))
                addedCount++;
            else
                duplicateCount++;
        } else {
            errorCount++; // неверный формат строки
        }
    }
    file.close();

    // Обновляем статус
    StatusBar1->SimpleText = "Загружено: " + IntToStr(addedCount) +
                             ", пропущено дубликатов: " + IntToStr(duplicateCount) +
                             ", ошибок формата: " + IntToStr(errorCount);
