...
std::vector<Route> CollectRoutes(const std::vector<City> &cities)
{
    ...
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
			if (ValidCity(cities, i, j)) {
				Route r;
				r.i = i;
				r.j = j;
				r.length = Distance(cities[i], cities[j]);
				result.push_back(r);
			}
		}
	}
...
//---------------------------------------------------------------------------
// Функция добавления города в список ListView
...
void __fastcall TForm1::PaintBox1Paint(TObject *Sender)
{
	...
	int activeIdxA = -1, activeIdxB = -1; // индексы в cities
	if (selectedRoute >= 0 && selectedRoute < (int)routes.size()) {
		...
		// Найти эти же города в общем списке cities
        for (int k = 0; k < (int)cities.size(); k++) {
	...
	// --- Города ---
	int r = 5; // радиус кружочка города
	for (int k = 0; k < (int)cities.size(); k++) {
		const City &c = cities[k];
		bool isChecked = ListView1->Items->Item[k]->Checked;
		int sx = toSX(c.x);
		int sy = toSY(c.y);
		bool isActive = (k == activeIdxA || k == activeIdxB); // город - часть выбранного маршрута
